module Twitter
  ...
  def self.oAuth(o={})
    ...
    parameter_string = signature_params.map{ |k, v| "%s=%s" % [k, v] }.join('&')
    ...
    # serialize
    "OAuth %s" % oauth_params.map {|k,v| '%s="%s"' % [k, v] }.join(", ")
  end
  ...
  def self.makeOAuthRequest(o={})
      ...
      query = url.path
      if o[:get_params] != {}
        query += '?' + o[:get_params].map { |param, value|
          "%s=%s" % [escape(param), escape(value)]
        }.join('&')
      end
